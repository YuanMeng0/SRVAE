...
from lvae.models.srvae.gradcam import GradCam
...
class VRLVBlockBase(nn.Module):
    ...
    def __init__(self, width, zdim, enc_key, enc_width, embed_dim=None, kernel_size=7, mlp_ratio=2):
        ...
        # 添加 InceptionDWConv2d 实例
        self.inception_dwconv = InceptionDWConv2d(width, square_kernel_size=3, band_kernel_size=11, branch_ratio=0.125)
...
#VRLVBlockSmall 类：这是潜在变量块的简化版本，它继承自 VRLVBlockBase 类，但具有更少的网络层，用于减少计算复杂度。
...
class VariableRateLossyVAE(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        self.gradcam_model=resnet50(True)
        self.cam=GradCam(self.gradcam_model)
    ...
    def forward_end2end(self, im: torch.Tensor, lmb: torch.Tensor, mode='trainval', get_latent=False):
        ...
        # zhongdian quyu shijia leijihuotu
        # x=self.cam(x)

        # ================ Forward pass ================
        _, enc_features = self.encoder(x.cuda(), emb)
        nB, _, xH, xW = x.shape
